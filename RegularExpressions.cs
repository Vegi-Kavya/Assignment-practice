...
namespace RegularExpressions
{
    class RegularExpressions
    {
        ...
        static void Main(string[] args)
        {
            ...
            //C# program that matches and parse a number
            string input4 = "Dot Net 200 Perls";
            Match match4 = Regex.Match(input4, @"\d+");
            if (match.Success)
            {
                int.TryParse(match4.Value, out int number);
                // Show that we have the numbers.
                Console.WriteLine("NUMBERS: {0}, {1}", number, number + 1);
            }

            string input2 = "kavya 100 percent loves jyothirmayee";
            Match Mch1 = Regex.Match(input2, @"\d+");
            {
                if (Mch1.Success)
                {
                    int.TryParse(Mch1.Value, out int number);
                    Console.WriteLine("Numbers: {0}, {1}", number, number + 1);
                }
            }

            // C# program value,length,index
            ...
            Match m1 = Regex.Match("jerry 123", @"j.*y");
            if (m1.Success)
            {
                Console.WriteLine("Value  = " + m1.Value);
                Console.WriteLine("Length = " + m1.Length);
                Console.WriteLine("Index  = " + m1.Index);
            }
